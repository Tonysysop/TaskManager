...
const TaskCard: React.FC<TaskCardProps> = ({ task }) => {
  return (
    <article className='w-full min-h-[100px] border border-gray-300 rounded-lg p-4 my-4'> 
      {/* Task Card */}
      <p className='text-lg font-semibold mb-3'>{task.task}</p> {/* Corrected task title */}

      <div className='flex items-center justify-between'>
        {/* Task Card Bottom */}
        <div>
          {task.tags.map((tag, index) => (
            <Tag key={index} tagName={tag} selectTag={() => {}} selected={true} /> // ✅ Fixed Tag props
          ))}
        </div>

        <div className='w-[35px] h-[35px] rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 ease-in-out hover:bg-gray-200'>
          <img className='w-[20px] opacity-50 transition-all duration-300 ease-in-out hover:opacity-80' src={DeleteIcon} alt="Delete Task" />
        </div> 
      </div>
    </article>
  )
