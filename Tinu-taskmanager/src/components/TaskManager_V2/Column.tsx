...
interface ColumnProps {
	...
	onDrop: (
		taskId: string,
		targetStatusValue: TaskAttributes["status"],
...
const Column: React.FC<ColumnProps> = ({
	...
	const handleColDrop = (e: React.DragEvent<HTMLDivElement>) => {
		e.preventDefault();
		setColOver(false);
	};
				...
				{tasks.length === 0 ? (
					...
					tasks.map((task, index) => (
						<React.Fragment key={task.id}>
							...
							>
								<TaskCard
									...
									handleChecklistToggle={(itemId, checked) =>
										onToggleChecklistItem(task.id, itemId, checked)
									}
								/>
							</motion.div>
							...
							<div className="">
								<DropArea
									onDrop={(taskId) => handleDropInColumn(taskId, index + 1)}
								/>
							</div>
						</React.Fragment>
					))
